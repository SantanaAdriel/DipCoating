...
int iniciando_temporario = 6;
int tempo_inicio;
int variavel_menu_inicio=0;
int inicio_contorno = 0;

// Variaveis mutaveis do ciclo (definir valores defolt)
int velocidade_backer;
int velocidade_forno;
int altura_backer;
long tempo_backer;
int altura_forno;
long tempo_forno;
...
void Iniciando() {
  if (iniciando_temporario == 6) {
    lcd.setCursor(0, 1);
    lcd.print("     Inicio     ");
    iniciando_temporario = 0;

  }
  if (millis() - tempo_inicio > 200) {
    iniciando_temporario = iniciando_temporario + 1;
    tempo_inicio = millis();
    lcd.setCursor(10 + iniciando_temporario, 1);
    lcd.print(">");
    lcd.setCursor(5 - iniciando_temporario, 1);
    lcd.print("<");
  }
}
...
void RemotoManual() {
  ...
  if (digitalRead(pino_botao_dir) == LOW && botao_dir_stado == 1) {
    if (millis() - tempo_menu_dir >= 280 ) {
      variavel_menu_inicio = inicio_contorno + 1;
  ...
  if (digitalRead(pino_botao_esq) == LOW && botao_esq_stado == 1) {
    if (millis() - tempo_menu_esq >= 280 ) {
      variavel_menu_inicio = inicio_contorno + 2;
      tempo_menu_max = millis();
      botao_esq_stado = 0;
    }
  }

  if (0 == variavel_menu_inicio) {
    inf_print_menu = variavel_menu_inicio;
    // Mostra o nome da opção do estagio 1 do menu
    lcd.setCursor(0, 0);
    lcd.print("Inicio remoto?");
    lcd.setCursor(0, 1);
    lcd.print("<SIM>      <NAO>");
  }


  switch (variavel_menu_inicio) {
    case 1: {

        // Conectar
        inicio_contorno = 2;
        inf_print_menu = 0;
        if (inf_print_menu != variavel_menu_inicio) {
          inf_print_menu = variavel_menu_inicio;
          // Mostra o nome da opção do estagio 1 do menu
          lcd.setCursor(0, 0);
          lcd.print("Conf. Parametros");
          lcd.setCursor(0, 1);
          lcd.print("<SIM>      <NAO>");
        }
      }
    ...
    case 3: {// Iniciar
      ...
      break;
    case 4: {// Configurar parametros
        lcd.clear();
        lcd.setCursor(0, 0);
        lcd.print("Mudar Parametros");
        etapa_do_ciclo = -1;
      }
      break;
  }
...
void setup() {
  ...
  velocidade_backer = 1; //velocidade de cm/s
  altura_backer = 5; //em cm a partir da base
  tempo_backer = 30; // em segundos

  velocidade_forno = 4; //velocidade de cm/s
...
void Menu() {
  ...
  if (estagio_menu == 1) {
    ...
    if (digitalRead(pino_botao_dir) == LOW && botao_dir_stado == 1) {
      if (millis() - tempo_menu_dir >= 280 ) {
        variavel_menu = (variavel_menu) % cond_contorno + 1; // Condiç-ao de contorno:::
        tempo_menu_max = millis();
        botao_dir_stado = 0;
      }
    }
    ...
    if (digitalRead(pino_botao_esq) == LOW && botao_esq_stado == 1) {
      if (millis() - tempo_menu_esq >= 280 ) {
        if (variavel_menu == 0) {
          variavel_menu = cond_contorno;
        }
        else {
          variavel_menu = (variavel_menu - 1) % cond_contorno; // Aplicar condição de contorno
        }
        tempo_menu_max = millis();
        botao_esq_stado = 0;
      }
    ...
    if (inf_print_menu != variavel_menu) {
      ...
      switch (variavel_menu) {
        ...
        case 2: {
            lcd.setCursor(1, 0);
            lcd.print("Mudar inform.");// COLOCAR A TEMPERATURA
            lcd.setCursor(0, 1);
            lcd.print("<<     ok     >>");
          }
          break;
  ...
  if (estagio_menu == 2) {
    ...
    switch (variavel_menu) {
      ...
      case 2: MudarVariaveis();
        break;
        
      case 3: MostraVariaveis();
        break;
...
void MostraVariaveis() {
  ...
  if (digitalRead(pino_botao_dir) == LOW && botao_dir_stado == 1) {
    if (millis() - tempo_menu_dir >= 280 ) {
      variavel_menu_display = (variavel_menu_display) % cond_contorno + 1; // Condiç-ao de contorno:::
      tempo_menu_max = millis();
      botao_dir_stado = 0;
    }
  }
  ...
  if (digitalRead(pino_botao_esq) == LOW && botao_esq_stado == 1) {
    if (millis() - tempo_menu_esq >= 280 ) {
      if (variavel_menu_display == 0) {
        variavel_menu_display = cond_contorno;
      }
      else {
        variavel_menu_display = (variavel_menu_display - 1) % cond_contorno; // Aplicar condição de contorno
      }
      tempo_menu_max = millis();
      botao_esq_stado = 0;
    }
  ...
  if (inf_print_menu != variavel_menu_display) {
    ...
    switch (variavel_menu_display) {
      case 0: {
          ...
          lcd.setCursor(0, 1);
          lcd.print("backer      cm/s");
          lcd.setCursor(8, 1);
          lcd.print(velocidade_backer, 2);
        }
        break;
      ...
      case 1: {
          ...
          lcd.print("Velocidade");
          lcd.setCursor(0, 1);
          lcd.print("forno       cm/s");
          lcd.setCursor(8, 1);
          lcd.print(velocidade_forno, 2);
        }
        break;
