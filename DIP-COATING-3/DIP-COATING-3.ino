...
void Menu() {
  ...
  if (estagio_menu == 1) {
    ...
    if (digitalRead(pino_botao_dir) == LOW && botao_dir_stado == 1) {
      if (millis() - tempo_menu_dir >= 280 ) {
        variavel_menu = (variavel_menu + 1) % cond_contorno; // Condiç-ao de contorno:::
        tempo_menu_max = millis();
        botao_dir_stado = 0;
      }
    }
    ...
    if (digitalRead(pino_botao_esq) == LOW && botao_esq_stado == 1) {
      if (millis() - tempo_menu_esq >= 280 ) {
        if (variavel_menu == 0) {
          variavel_menu = cond_contorno - 1;
        }
        else {
          variavel_menu = (variavel_menu - 1) % (cond_contorno - 1); // Aplicar condição de contorno
        }
        tempo_menu_max = millis();
        botao_esq_stado = 0;
      }
  ...
  if (estagio_menu == 2) {
    ...
    switch (variavel_menu) {
      ...
      case 2: MudarVariaveis(); // retirnornar ao menu
        break;
...
void MostraVariaveis() {
  ...
  if (digitalRead(pino_botao_dir) == LOW && botao_dir_stado == 1) {
    if (millis() - tempo_menu_dir >= 280 ) {
      variavel_menu_display = (variavel_menu_display + 1) % cond_contorno; // Condiç-ao de contorno:::
      tempo_menu_max = millis();
      botao_dir_stado = 0;
    }
  }
  ...
  if (digitalRead(pino_botao_esq) == LOW && botao_esq_stado == 1) {
    if (millis() - tempo_menu_esq >= 280 ) {
      if (variavel_menu_display == 0) {
        variavel_menu_display = cond_contorno - 1;
      }
      else {
        variavel_menu_display = (variavel_menu_display - 1) % (cond_contorno - 1); // Aplicar condição de contorno
      }
      tempo_menu_max = millis();
      botao_esq_stado = 0;
    }
